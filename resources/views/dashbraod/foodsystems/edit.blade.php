...
<div class="container">
    ...
    <div class="card-body">
        ...
        <form action="{{route('dashbraod.info0.edit', $info0->id)}}" method="post" enctype="multipart/form-data">
            @csrf 
            @method('PUT')
            <div class="mb-3">
                <label for="name">title:</label>
                <input type="text" class="form-control" id="title" name="title" value="{{$info0->title}}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="posts_title" name="posts_title" value="{{$info0->posts_title}}">
                @error('posts_title')<span  class="text-danger">{{$message}}</span> @enderror

            </div>
            <div class="mb-3">
                <label for="posts_img">posts_img</label>
                <input type="file" class="form-control" id="posts_img" name="posts_img" value="{{$info0->posts_img}}">
                @error('posts_img')<span  class="text-danger">{{$message}}</span> @enderror

            </div>
            <div class="mb-3">
                <label for="posts_url">posts_url</label>
                <input type="posts_url" class="form-control" id="posts_url" name="posts_url" value="{{$info0->posts_url}}">
                @error('posts_url')<span  class="text-danger">{{$message}}</span> @enderror

            </div>
            <div class="mb-3">
                <label for="posts_content">posts_content</label>
                <input type="text" class="form-control" id="posts_content" name="posts_content" value="{{$info0->posts_content}}">
            ...
            <div class="mb-3">
                ...
                <input type="url" class="form-control"   id="url" name="url" value="{{$info0->url}}">
